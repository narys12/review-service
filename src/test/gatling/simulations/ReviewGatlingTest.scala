...
class ReviewGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Review entity")
        ...
        .repeat(2) {
            ...
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "title":"SAMPLE_TEXT", "comment":"SAMPLE_TEXT", "created":"2020-01-01T00:00:00.000Z", "published":"2020-01-01T00:00:00.000Z", "star":"0", "pros":"SAMPLE_TEXT", "cons":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_review_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created review")
                .get("/review${new_review_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created review")
            .delete("/review${new_review_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
